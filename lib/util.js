...
util.buildTplParams = function (t, obj){
  let currentPage = t.getCurrentPages();
    ...
    if (button['action'] == 'url') {
      ...
      var postFun = function () {
        util.getDSData(ds, valueobj, function (map) {
          ...
          //触发按钮执行完成事件
          t.triggerEvent("button-finish", button, valueobj, map);
          t.triggerEvent("button-finish-success", button, valueobj, map);
        }, function (err, msg) {
          wx.showToast({
          ...
          reject();
          //触发按钮执行完成事件
          t.triggerEvent("button-finish", button, valueobj, err);
          t.triggerEvent("button-finish-error", button, valueobj, err);
        });
      }
      if (button['noConfirm']) {
        postFun();
