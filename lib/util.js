...
/**
 * 因为小程序每次请求sessionid 都是不同的，所以这里设置cookie 保证每次请求session相同
 */
var invoke = function (options){
  return new Promise((resolve,reject)=>{
    //先拿storage的sessionId
    var cookie = wx.getStorageSync("_TY_s");
      wx.request({
        ...
        success: function (res) {
          if (res && res['data'] && res['data']['code'] && res['data']['code'] <= -400) {
            //session失效
            wx.navigateTo({
              url: app.globalData._TY_SSOURL
            });
          }
          resolve(res);
        },
...
util.loadBuzz = function (buzz, handle) {
    ...
    //     console.log(err);
    // });
    const buzzFun = require("../package/buzz/"+buzz+".js");
    if (handle){
      handle(buzzFun);
    }else{
      return buzzFun;
...
util.getDSData = function(ds, inputValueObj, success, error) {
    ...
    util[method](apiUrl, requestParam).then((response)=>{
      wx.hideLoading();
          ...
          outputs.forEach(function(output) {
            ...
            if (output['handle']) {
              ...
              const item = new Promise((resolve, reject) => {
                output['value'] = util.loadBuzz(output['handle'])({
  ...
  */
  util.resolveButton = function(button, valueobj) {
    ...
    return new Promise((resolve, reject) => {
      if (button['action'] == 'url') {
        ...
        //如果是巴斯代码，远程加载
        util.loadBuzz(button.buzz, function (fun) {
          //调用buzz
          fun(valueobj, button,t);
          resolve();
          //触发按钮执行完成事件
          t.triggerEvent("button-finish", {
            button: button,
        ...
        //触发按钮执行完成事件
        t.triggerEvent("button-finish", { button: button, valueobj:valueobj});
        console.log("按钮解析类型不支持!");
        // wx.showToast({
        //   title: "不支持",
        //   icon: 'none',
        //   duration: 2000
  ...
  util.getCurrentUrl = function() {
    ...
    //因为native的tabbar跳转不支持navigate，所以跳转的页面不能是tabbar的页面
    urlWithArgs = urlWithArgs.replace("/pages/index/home", "/pages/index/index").replace("/pages/index/my", "/pages/index/index").replace("/pages/index/news", "/pages/index/index");
    return urlWithArgs
  },
util.scanQRCode = function (successFn) {
  wx.scanCode({
    onlyFromCamera: false,//只能从相机扫码
...
// 编码字符串  小程序生成二维码不支持%号  小程序二维码生成参数只支持32个字符
util.encode=function(val){
  return val.replace("?alias=","");//把前面的别名去除
}
//解码字符串
util.decode=function(val){
  // let tmpStr = val.replace(/__/gi, "%");
