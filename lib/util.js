    ...
    if (button['action'] == 'url') {
      ...
      var postFun = function () {
        ...
        util.getDSData(ds, valueobj, function (map) {
          ...
          //触发按钮执行完成事件
          t.triggerEvent("button-finish", { button: button, valueobj: valueobj, map: map });
          t.triggerEvent("button-finish-success", { button: button, valueobj: valueobj, map: map });
        }, function (err, msg) {
          wx.showToast({
          ...
          reject();
          //触发按钮执行完成事件
          t.triggerEvent("button-finish", { button: button, valueobj:valueobj, err:err});
          t.triggerEvent("button-finish-error", { button: button, valueobj:valueobj, err:err});
        });
      }
      if (button['noConfirm']) {
        postFun();
